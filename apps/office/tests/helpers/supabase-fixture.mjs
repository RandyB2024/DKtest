...
export function supabaseFixture() {
  const state = {
    password:randomUUID(), aal:'aal2', sessions:new Map(), calls:[], ttl:3600,
  ...
  function session(aal = state.aal) {
    const now = Math.floor(Date.now()/1000), encode = value => Buffer.from(JSON.stringify(value)).toString('base64url');
    const token = `${encode({ alg:'HS256',typ:'JWT' })}.${encode({ sub:state.user.id,aal,exp:now+state.ttl,iat:now,session_id:randomUUID(),role:'authenticated' })}.${Buffer.from(randomUUID()).toString('base64url')}`;
    const refresh = randomUUID(); state.sessions.set(token,{ refresh,aal });
  ...
  state.fetch = async (input,init={}) => {
    ...
    if (url.pathname === '/auth/v1/token') {
      if (url.searchParams.get('grant_type') === 'refresh_token') {
        ...
        state.ttl=3600; return json(session(previous.aal));
      }
    ...
    if (url.pathname.endsWith('/verify')) {
      ...
      state.user.factors.forEach(f => {f.status='verified';}); return json(session('aal2'));
    }
