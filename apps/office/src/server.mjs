...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  console.log(`Database: ${health.database.toUpperCase()}`);
  console.log(`Migrations: ${health.migrations.toUpperCase()}`);
  console.log(`Demo data: ${health.demoData.toUpperCase()}`);
  console.log(`Session store: ${health.sessionStore.toUpperCase()}`);
  if (health.status !== 'ok') throw new Error('Startup health check failed.');
  createServer().listen(config.port, '127.0.0.1', () => console.log(`Destination Known Office: http://127.0.0.1:${config.port}`));
}
