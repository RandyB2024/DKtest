...
test("PostgreSQL executes foundation + migration and enforces customer RLS and AAL2", async () => {
  const pg = new PGlite();
  try {
    ...
    // from both migrations is executed without modification.
    const foundation = readFileSync(new URL("../supabase/migrations/202609230001_initial_test_foundation.sql", import.meta.url), "utf8");
    await pg.exec(foundation.replace("create extension if not exists pgcrypto;", ""));
    await pg.exec(readFileSync(new URL("../supabase/migrations/202609240001_portal_auth_boundary.sql", import.meta.url), "utf8"));
